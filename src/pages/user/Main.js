...
const Main = () => {
    ...
    const getPopupList = () => {
        axios
            .get(`${auth_popup_list}?p_type=${popupType}`)
            .then((res) => {
                if (res.status === 200) {
                    ...
                    let openPopList = list.filter(
                        (item) => item.p_layer_pop[0] == 2
                    );
                    //쿠키에 저장된 오늘은그만보기 팝업제외하고 노출
                    openPopList = openPopList.filter(
                        (item) => !hideList.includes(item.idx)
                    );
                        ...
                        openPopList.forEach((item) => {
                            ...
                            if (popupWindow) {
                                ...
                                popupWindow.document.write(
                                    `<script>window.location.href = "${popupUrl}";</script>`
                                );
                            }
                        });
                    };
    ...
    const getNewsList = () => {
        axios
            .get(`${board_list.replace(":category", 49).replace(":limit", 8)}`)
            .then((res) => {
                if (res.status === 200) {
    ...
    return (
        <>
            ...
            {/* 관리자단에서 설정한 팝업 */}
            <UserPop list={popupList} mo={popupType == "M" ? true : false} />
