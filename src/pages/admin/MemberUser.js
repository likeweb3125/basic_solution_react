...
const MemberUser = () => {
    ...
    const [levelNameSelect, setLevelNameSelect] = useState('');
    ...
    const getBoardData = (page) => {
        ...
        axios.get(`${member_list}?page=${page ? page : 1}&getLimit=${limit}${levelSelect ? '&m_level='+levelSelect : ''}${searchTxt.length > 0 ? "&search="+search+"&searchtxt="+searchTxt : ""}${mailCheck ? "&m_mail_yn=Y" : ""}${smsCheck ? "&m_sms_yn=Y" : ""}${startDate ? "&sdate="+sdate : ""}${endDate ? "&edate="+edate : ""}`,
            {headers:{Authorization: `Bearer ${user.loginUser.accessToken}`}}
        )
        .then((res)=>{
            if(res.status === 200){
                let data = res.data.data;
                    data.member_list = data.member_list.filter((item) => item.m_level !== 9); //관리자회원 제외
                setBoardData(data);

                //단체문자 회원수
                setSmsMember(data.member_list.length);
            }
        })
        .catch((error) => {
            const err_msg = CF.errorMsgHandler(error);
    ...
    useEffect(()=>{
        ...
        if(num > 0){
            ...
            memberNum = boardData.member_list && boardData.member_list.length;
            setSmsMember(memberNum);
        }
    },[etc.checkedList]);
